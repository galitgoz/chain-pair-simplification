...
Root-level `0.*` and `1.*` notebooks and remaining `build_*` and `execute_*` scripts are earlier workflows, not an ordered current reproduction pipeline. Twelve historical maintenance helpers now live in [scripts/legacy](../scripts/legacy/README.md). Core modules and builders with active dependencies retain their locations.
...
The remaining historical notebooks and saved evidence retain their locations because they may depend on relative paths.
