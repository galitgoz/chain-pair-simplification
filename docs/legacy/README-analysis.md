...
To regenerate this version, run `src/execute_decimation_notebook.py`. The older `src/execute_small_notebook.py` builds the superseded 30-residue experiment and should not be used to rebuild the current notebook.
...
The existing `data/raw protein backbones` files are unchanged. Cached primary PDB/mmCIF records augment them for residue identities and missing-region verification. If the cache is absent, run `src/prepare_sources.py` (network access required).
...
`src/curve_algorithms.py` supplies the independently testable implementation, also embedded in the notebook. `src/protein_data.py` handles traceable preparation. `src/verify_algorithms.py` checks small-instance global optimality, degenerate geometry, and metric/endpoint differences. `src/benchmark_fragments.py` runs the 69-case independent sweep. `src/build_notebook.py` generates the notebook source; `src/execute_notebook.py` validates the stored sweep and executes it with the current interpreter.
