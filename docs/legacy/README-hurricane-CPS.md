...
Open `notebooks/legacy/1.analyze_CPS.ipynb` from the project root and run its cells in order with
the project Python environment. It reuses `src/curve_algorithms.py` and
`src/cps_paper_algorithms.py`; hurricane loading, experiments and plots live in
`src/hurricane_cps.py`.
...
`src/build_hurricane_cps_notebook.py` recreates the notebook template and overwrites
