...
Open `notebooks/legacy/0.analyze_CPS.ipynb` from this project directory and run its cells in order.
...
The notebook's data and plotting helpers live in `src/cps_notebook_data.py`,
`src/cps_notebook_run.py`, and `src/cps_notebook_plots.py`. The builder embeds algorithm
source from `src/curve_algorithms.py` and `src/cps_paper_algorithms.py`.
...
executes from a fresh kernel. The older `src/execute_cps_notebook.py` instead regenerates
from `src/build_cps_notebook.py`, replacing manual notebook edits with the template.
