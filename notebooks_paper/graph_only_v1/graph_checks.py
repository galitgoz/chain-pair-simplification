...
import numpy as np
import pandas as pd
from graph_core import ROOT,OUT,METHODS,solve,measures
...
def check_all():
    ...
    for m in ['CPS-2F','CPS-3F']:
        def route(method=m):
            result=solve(line,line,1,1,1,method,certificate=False)
            assert result['route']=='configuration_graph' and result['configuration_graph_entered']
            assert result.get('states')>0 and result.get('transitions')>0
            return 'Independently feasible outputs do not bypass execution: both chain graphs and the product recurrence ran.'
        record('feasible_independent_outputs_do_not_bypass_graph',m,route)
...
if __name__=='__main__':
    ...
    (OUT/'shortcut_guard.json').write_text(json.dumps(dict(alternate_subsequence_calls=forbidden.call_count,
        certificate=False,independent_shortcut_bypassed_on_feasible_examples=True,
        graph_entry_observation='first heap pop directly inside solve_cps; baseline single-curve graphs excluded'),indent=2))
