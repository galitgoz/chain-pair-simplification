...
try:
    ...
    if config.get('validation_replay_raw'):
        source=ROOT/config['validation_replay_raw']
        assert sha(source)==config['validation_replay_sha256']
        saved=json.loads(source.read_text())
        assert saved['input_sha256']==config['input_sha256']
        for key in ['method','alpha','delta1','delta2','delta3']:
            assert saved['configuration'][key]==config[key]
        atomic(DIRECTORY/'replay_source.json',dict(raw_checkpoint=str(source.relative_to(ROOT)),sha256=sha(source),optimization_executed=False))
        phases.begin('validation');start=time.perf_counter()
        try:
            checked=validate(A,B,saved['raw_result'],config)
            checked.update(validation_seconds=time.perf_counter()-start,raw_checkpoint_sha256=sha(source),recovery=True,**stamp())
            ...
            atomic(DIRECTORY/'validation.json',dict(status='validation_failed',validation_seconds=time.perf_counter()-start,
                failure_reason=str(error),failure_kind=classify(error),raw_checkpoint_sha256=sha(source),**stamp()))
        phases.begin('reporting');atomic(DIRECTORY/'worker_finished.json',dict(status='validation_replay_finished',optimization_executed=False,**stamp()))
        raise SystemExit(0)
    ...
    checkpoint=dict(schema_version=3,certificate_enabled=False,configuration=config,configuration_sha256=sha(DIRECTORY/'configuration.json'),
