...
import pandas as pd
import nbformat
root=Path(__file__).resolve().parents[1];out=root/(sys.argv[1] if len(sys.argv)>1 else 'output/hurricane_auxiliary')
nb=nbformat.read(root/'notebooks/legacy/0.analyze_hurricans.ipynb',4)
