...
path=root/'notebooks/legacy/0.analyze_CPS.ipynb';nb=nbformat.read(path,as_version=4)
...
def progress(cell,cell_index,**kwargs):
    print(f'Cell {cell_index+1}/{len(nb.cells)}: {cell.source.splitlines()[0][:80]}'.encode('ascii','replace').decode(),flush=True)
...
print(f'Executed {path.name}: {path.stat().st_size} bytes',flush=True)
