...
root=Path(__file__).resolve().parent;out=root/(sys.argv[1] if len(sys.argv)>1 else 'output/hurricane_cps')
path=root/'1.analyze_CPS.ipynb';nb=nbformat.read(path,4)
...
manifest['sha256']['hurricane_cps.py']=hashlib.sha256((root/'hurricane_cps.py').read_bytes()).hexdigest()
